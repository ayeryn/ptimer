...
export function seedIfNeeded() {
  ...
  if (deadliftIdx >= 0) {
    ...
    const existingExerciseIds = (routines[deadliftIdx].exercises ?? []).map(
      (exercise) => exercise.id,
    );
    if (legacyExerciseIds.every((id) => existingExerciseIds.includes(id))) {
      const deadliftPreset = PRESET_ROUTINES.find(
        (r) => r.id === "preset-deadlift",
      );
      if (deadliftPreset) {
  ...
  const singleSideIdx = routines.findIndex(
    (routine) => routine.id === "preset-single-side-tricep-extension",
  );
  if (singleSideIdx >= 0) {
    const exerciseIds = (routines[singleSideIdx].exercises ?? []).map(
      (exercise) => exercise.id,
    );
    if (
      exerciseIds.length === 1 &&
      exerciseIds[0] === "ex-single-side-tricep-extension"
    ) {
      const preset = PRESET_ROUTINES.find(
        (routine) => routine.id === "preset-single-side-tricep-extension",
      );
      if (preset) {
